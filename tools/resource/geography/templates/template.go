...
//AllGeographyTemplates returns slice of all geography templates ready to render
func AllGeographyTemplates(variables templateUtils.TemplateVariables) []Template {
	return []Template{
		NewEntity(variables),
		NewControllerGet(variables),
		NewControllerDelete(variables),
		NewControllerGetList(variables),
		NewControllerPost(variables),
		NewControllerPut(variables),
		NewResource(variables),
		NewRepository(variables),
		NewRestApiDelete(variables),
		NewRestApiGetList(variables),
		NewRestApiPost(variables),
		NewRestApiPut(variables),
		NewBehatCreate(variables),
		NewBehatGetId(variables),
		NewBehatDelete(variables),
		NewBehatGetList(variables),
		NewBehatPut(variables),
		NewDocumentationRequest(variables),
		NewDocumentationResponseSingle(variables),
		NewDocumentationResponseArray(variables),
	}
}
